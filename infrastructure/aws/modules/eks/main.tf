# EKS Cluster Module for Rental ML System
...
resource "aws_security_group" "cluster" {
  ...
  dynamic "ingress" {
    ...
    content {
      ...
      security_groups = lookup(ingress.value, "source_node_security_group", false) ? [aws_security_group.node_group_one.id] : lookup(ingress.value, "security_groups", null)
    }
  }
...
resource "aws_eks_addon" "addons" {
  ...
  cluster_name      = aws_eks_cluster.main.name
  addon_name        = each.key
  addon_version     = lookup(each.value, "most_recent", false) ? data.aws_eks_addon_version.latest[each.key].version : lookup(each.value, "version", null)
