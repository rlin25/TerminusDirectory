"""
Production-ready Rentals.com scraper implementation.

This module provides a comprehensive scraper for rentals.com property listings
...
class ProductionRentalsComScraper(ProductionBaseScraper):
    """Production-ready scraper for rentals.com listings"""
    
    def __init__(self, config=None):
        super().__init__('rentals_com', config)
        self.base_url = "https://www.rentals.com"
    ...
    async def get_listing_urls(self, base_url: str, max_pages: int = None) -> AsyncGenerator[str, None]:
        ...
        while page <= max_pages:
            try:
                # Construct search results URL for rentals.com
                if page == 1:
                    search_url = base_url
                else:
                    search_url = f"{base_url}?page={page}"
                
                logger.debug(f"Fetching rentals.com search results page {page}: {search_url}")
    ...
    def _extract_listing_links(self, soup: BeautifulSoup) -> List[str]:
        ...
        try:
            ...
            listing_selectors = [
                '.listing-item a[href*="/rental/"]',
                '.property-card a[href*="/rental/"]',
                'a[href*="/property/"]',
                '.rental-listing a',
                '.listing-card a[href*="/rental/"]'
            ]
            ...
            for selector in listing_selectors:
                ...
                for element in elements:
                    ...
                    if href and ('/rental/' in href or '/property/' in href):
