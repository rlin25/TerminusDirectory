"""
Production-ready Rentals.com scraper.

This module provides a robust scraper for rentals.com with comprehensive 
...
class RentalsComScraper(ProductionBaseScraper):
    """Production scraper for rentals.com listings"""
    
    def __init__(self, config=None):
        super().__init__('rentals_com', config)
        
        # Rentals.com specific selectors
        self.selectors = {
            'listing_links': [
                'a[href*="/rental/"]',
                '.listing-card a',
    ...
    def get_search_urls(self) -> List[str]:
        """Generate search URLs for Rentals.com"""
        search_urls = []
        base_url = self.source_config.base_url
        
        # Map locations to Rentals.com format
        location_mapping = {
        ...
        for location in self.source_config.search_locations:
            rentals_location = location_mapping.get(location, location)
            search_urls.append(f"{base_url}/search/{rentals_location}")
            
            # Add filtered searches for major cities
            if location in ['new-york-ny', 'los-angeles-ca', 'chicago-il']:
                search_urls.append(f"{base_url}/search/{rentals_location}?type=apartment")
                search_urls.append(f"{base_url}/search/{rentals_location}?min_price=1500")
        
        logger.info(f"Generated {len(search_urls)} search URLs for rentals.com")
        return search_urls
    
    async def get_listing_urls(self, base_url: str, max_pages: int = None) -> AsyncGenerator[str, None]:
        """Extract listing URLs from Rentals.com search results"""
        ...
        while page <= max_pages and consecutive_empty_pages < 3:
            try:
                ...
                logger.debug(f"Fetching Rentals.com page {page}: {search_url}")
                ...
                logger.error(f"Error processing Rentals.com page {page}: {e}")
                consecutive_empty_pages += 1
                page += 1
        ...
        for selector in self.selectors['listing_links']:
            ...
            for element in elements:
                ...
                if href and '/rental/' in href:
                    ...
                    if href.startswith('/'):
                        links.append(href)
                    elif href.startswith('http') and 'rentals.com' in href:
    ...
    async def extract_property_data(self, listing_url: str, html_content: str) -> Optional[Property]:
        """Extract property data from Rentals.com listing"""
        try:
            soup = BeautifulSoup(html_content, 'html.parser')
            ...
            logger.error(f"Error extracting Rentals.com property data from {listing_url}: {e}")
            self.metrics.properties_invalid += 1
            return None
    ...
    def _extract_title(self, soup: BeautifulSoup) -> Optional[str]:
        ...
        if title_tag:
            ...
            title = re.sub(r'\s*\|\s*Rentals\.com.*$', '', title, flags=re.IGNORECASE)
            if title:
                return title
    ...
    def _extract_price(self, soup: BeautifulSoup) -> Optional[float]:
        ...
        for selector in self.selectors['price']:
            ...
            for element in elements:
                ...
                # Price patterns for Rentals.com
                price_patterns = [
                    r'\$(\d{1,2},?\d{3,4})',
                    r'(\d{1,2},?\d{3,4})\s*/\s*month',
                    r'from\s+\$(\d{1,2},?\d{3,4})',
                    r'starting\s+\$(\d{1,2},?\d{3,4})'
                ]
    ...
    def _extract_amenities(self, soup: BeautifulSoup) -> List[str]:
        ...
        # Normalize amenities using parent class method
